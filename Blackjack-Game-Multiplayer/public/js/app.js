...
function playerBets() {
  for (let b = 0; b < betButtons.length; b++) {
    ...
    betButtons[b].addEventListener("click", () => {
      ...
      if (pendingBet + betAmount <= theClient.balance) {
        // Chips stay in the betting tray until PLACE BET commits them.
        pendingBet = pendingBet + betAmount;
      } else {
...
$(document).on("click", ".ready", function () {
  if (pendingBet <= 0 || pendingBet > theClient.balance) return;

  chipPlace.play();
  theClient.bet = pendingBet;
  theClient.balance = theClient.balance - pendingBet;
  pendingBet = 0;
...
function showDecisionUI() {
  ...
  $("#decision-panel-toggle")
    .removeClass("hide-element is-cards-view")
    .text("View cards");
}

function hideDecisionUI(allowRestore = false) {
  $(".user-action-container").addClass("hide-element");
  $("#dark-overlay").css("opacity", "");
  if (allowRestore) {
    $("#decision-panel-toggle")
      .removeClass("hide-element")
      .addClass("is-cards-view")
      .text("Make decision");
  } else {
    $("#decision-panel-toggle").addClass("hide-element");
  }
}

function resumePlayerDecision() {
  // Keep the newly dealt card clear long enough for its animation and value
  // to be read before asking the player for another action.
  setTimeout(sendPlayerThePlay, 900);
}
...
// UPDATE CSS BET DRAFT WITHOUT CHARGING THE PLAYER
$(".update-balance-bet").click(function () {
  updatePendingBetUI();
});

// CLEAR AND MAX EDIT ONLY THE UNCOMMITTED BETTING TRAY
$(".max-clear").click(function () {
  defaultClick.play();
  if (this.innerText === "CLEAR") {
    pendingBet = 0;
  } else if (this.innerText === "MAX") {
    if (theClient.balance <= 0) {
      alert("Need more balance");
    } else {
      pendingBet = theClient.balance;
    }
  }
  updatePendingBetUI();
});

$("#decision-panel-toggle").click(function () {
  if ($(".user-action-container").hasClass("hide-element")) {
    showDecisionUI();
  } else {
    hideDecisionUI(true);
  }
